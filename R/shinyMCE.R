...
tinyMCE <- function(inputId, content, options = NULL){
  ...
  apikey <- getOption("shinto_mce_apikey")
  if(is.null(apikey)){
    stop("Get API key for TinyMCE and set options(shinto_mce_apikey = 'KEY')")
  }
  ...
  tagList(
    ...
    singleton(
      tags$head(
        tags$script(src = paste0("https://cdn.tiny.cloud/1/",apikey,"/tinymce/5/tinymce.min.js"))
      )
    ),
    tags$form(method = "post",
      tags$div(style = "width: 100%; height: 300px; padding: 20px; border: 1px solid black;",
        id = inputId, class = "shinytinymce",
      )
    ),
    # tags$div(id = inputId, class = "shinytinymce", 
    #          content, 
    #          style = "resize: none; width: 100%; height: 100%; border-style: none; background: gainsboro;"),
    tags$script(paste0("tinymce.init({selector:'#",inputId,"', inline: true})")),
